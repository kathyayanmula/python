...
class Ship:
    ...
    def update(self):
        global thrust, thrust_vel, vector
        self.angle += self.angle_vel
        vector = angle_to_vector(self.angle)
        if self.thrust:
            acceleration = .8
            self.vel[0] = self.vel[0] + acceleration * vector[0]
            self.vel[1] = self.vel[1] + acceleration * vector[1]
        deceleration = 0.95
        self.vel[0] = self.vel[0] * deceleration
        self.vel[1] = self.vel[1] * deceleration
    ...
    def shoot(self):
        global vector, a_missile
        a_missile = Sprite([self.pos[0] + vector[0] * 35, self.pos[1] + vector[1] * 35],
                           [self.vel[0] + 2 * vector[0], self.vel[1] + 2 * vector[1]], 0, 0,
...
def draw(canvas):
    ...
    chances = 'Lives:' + str(lives)
    canvas.draw_text(points, [30, 30], 28, 'red')
    canvas.draw_text(chances, [700, 30], 28, 'red')
...
def rock_spawner():
    global a_rock
    a_rock = Sprite([random.randint(400, WIDTH), random.randint(0, HEIGHT)],
                    [random.choice([.5, -.5]), random.choice([.5, -.5])],
                    0, random.choice([-.1, .1]), asteroid_image, asteroid_info)
